...
def verify_payment(request):
    ...
    if order.payment_method != 'cod':
        # Online payment → mark full as paid
        client = razorpay.Client(auth=(settings.RAZORPAY_KEY_ID, settings.RAZORPAY_KEY_SECRET))
        try:
...
def admin_dashboard(request):
    ...
    # Total revenue is sum of paid amounts
...
def print_orders_by_date(request):
    date_str = request.GET.get('date')  # dashboard passes ?date=YYYY-MM-DD
    if date_str:
        date_obj = parse_date(date_str)
        if date_obj:
            orders = Order.objects.filter(created_at__date=date_obj)
            # Add dynamic balance_amount
            for order in orders:
...
def print_all_orders(request):
    ...
    for order in orders:
        balance_amount = Decimal('0.00')
        if order.payment_method == 'cod' and order.payment_status != 'completed':
            # 20% already paid, remaining 80% to collect
            balance_amount = (order.total_amount * Decimal('0.8')).quantize(Decimal('0.01'))
